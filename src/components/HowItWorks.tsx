      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {steps.map((step, index) => (
            <div key={index} className={`text-center group animate-fade-in animate-stagger-${index + 1}`}>
              <div className="relative mb-6">
                <div className={`w-16 h-16 mx-auto rounded-full bg-gradient-to-br ${step.color} flex items-center justify-center mb-4 hover-scale animate-float`} style={{animationDelay: `${index * 0.8}s`}}>
                  <step.icon className="w-8 h-8 text-white" />
                </div>
                <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-2 text-sm font-bold text-accent animate-pulse">
                  {index + 1}
                </div>
              </div>
              ...
              {index < steps.length - 1 && (
                <div className="hidden lg:block absolute top-8 left-full w-8 h-0.5 bg-gradient-to-r from-accent/50 to-transparent transform -translate-x-4 animate-fade-in" style={{animationDelay: `${index * 0.8 + 0.4}s`}} />
              )}
            </div>
          ))}
        </div>
